...
| `--resampling`  | `bilinear`    | Interpolation method: `bilinear`, `nearest`        |
...
High-quality WebP with nearest-neighbor resampling:

```bash
./geotiff2pmtiles --format webp --quality 95 --resampling nearest \
  data/ output.pmtiles
```
...
internal/
  ...
  tile/
    ...
    resample.go                     Bilinear/nearest interpolation + reprojection
...
6. **Resample**: Bilinear or nearest-neighbor interpolation from source COG tiles (cached)
